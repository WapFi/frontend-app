...
function RepaymentsSection() {
    ...
    fetchRepayments()
      .then((res) => {
        if (res.status && res.data.repayments.length > 0) {
          ...
          console.log("my repayments: ", res.data.repayments)
  ...
  return (
    <div className="rounded-xl flex flex-col self-stretch justify-center w-full items-start gap-6 border-[rgba(255,255,255,0.80)] border-1 bg-[#fff]">
      {/* DESKTOP HEADER */}
      <div className="hidden md:flex justify-between items-start self-stretch w-full gap-6">
        ...
        <div className="md:w-[50%] flex justify-between items-center gap-10">
          <div className="flex items-center justify-between gap-2.5 pl-3 shrink-0 rounded-[30px] w-full md:w-[70%] border border-[rgba(0,0,0,0.08)] bg-[rgba(255,255,255,0.80)]">
            <input
              type="search"
