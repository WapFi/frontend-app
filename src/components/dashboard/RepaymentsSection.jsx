...
function RepaymentsSection({totalLoansTaken}) {
  ...
  const fetchFilteredRepayments = async (filters = {}) => {
    ...
    try {
      ...
      if (res?.status) {
        const data = res.data.repayments || [];
        setRepayments(data);
  ...
  const noRepayments = repayments.length === 0;
    ...
    <div className="rounded-xl flex flex-col self-stretch justify-center w-full items-start gap-6 bg-[#fff] lg:p-6">
        ...
        >
          ...
          >
            <input
              ...
              disabled={noLoansTaken}
            />
            <button className="cursor-pointer" onClick={handleApplyFilters} disabled={noLoansTaken}>
              <img src={searchIcon} alt="search" />
            </button>
          </div>
          <Link
            to={noLoansTaken ? "#" : "/repayments"}
      ...
      <div className="w-full flex items-start justify-between">
        {error ? (
          ...
          <RepaymentsList repayments={repayments} onSelect={handleSelectLoan} />
        ) : loanDetailsLoading ? (
          <PageLoader />
        ) : (
