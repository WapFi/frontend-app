...
export default function SettingsChangePassword() {
  ...
  // Use settingsChangePassword.* for translation keys
  const schema = yup.object({
    currentPassword: yup
      .string()
      .required(t("settingsChangePassword.errors.password_required"))
  ...
  return (
    <div className="rounded-[24px] w-full flex flex-col gap-8 lg:gap-5 mb-10 lg:mb-16 lg:bg-white md:w-[85%] md:mx-auto md:mt-8 lg:mt-0 lg:w-full lg:px-7 lg:pt-9 lg:pb-20">
      <p className="text-[#222] font-semibold text-[24px] font-raleway lg:text-[28px] lg:font-bold lg:text-[#10172E]">
        {t("settingsChangePassword.title")}
      </p>
      <form
      ...
      >
        ...
        <div>
          <label className="text-[#222] gap-2">
            {t("settingsChangePassword.current")} <br />
            <div className="relative w-full">
              <input
                {...register("currentPassword")}
                type={showCurrentPassword ? "text" : "password"}
                placeholder={t("settingsChangePassword.placeholders.current")}
          ...
          <Link
            ...
            className="text-[#439182] block cursor-pointer text-right mt-2"
          >
            {t("settingsChangePassword.forgot")}
          </Link>
        </div>

        {/* New Password */}
        <label className="text-[#222] gap-2">
          {t("settingsChangePassword.new")} <br />
          <div className="relative w-full">
            <input
              {...register("newPassword")}
              type={showNewPassword ? "text" : "password"}
              placeholder={t("settingsChangePassword.placeholders.password")}
              className="text-[rgba(34,34,34,0.50)] border-[#00000026] w-full gap-3 border-1 rounded-lg p-[14px]"
            />
            <button
              type="button"
        ...
        <label className="text-[#222] gap-2">
          {t("settingsChangePassword.confirm")} <br />
          <div className="relative w-full">
            <input
        ...
        >
          {loading ? <LoadingSpinner /> : t("settingsChangePassword.save")}
        </button>
      </form>
    </div>
  );
