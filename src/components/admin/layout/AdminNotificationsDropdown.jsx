...
export default function AdminNotificationsDropdown({ onNotificationAction }) {
  ...
  // CHANGED: isProcessingAction now holds the ID of the notification
  ...
  const handleMarkAsRead = async (notificationId) => {
    // UPDATED: Set the specific notification ID as the active action
    setIsProcessingAction(notificationId);
    ...
    try {
      ...
      // UPDATED: Clear the active action ID
  ...
  return (
    <div className="absolute -right-1 mt-2 w-80 bg-white rounded-lg shadow-lg border border-gray-200 z-50">
      <div className="p-4 border-b border-gray-200 flex flex-col items-start">
        <h3 className="text-[20px] font-raleway font-semibold text-[#171717] mb-3">Notifications</h3>
        <div className="flex gap-2.5 text-sm">
          {unreadCountInDropdown > 0 && (
            <button
              className="text-[#B88E00] font-medium cursor-pointer"
              onClick={handleMarkAllAsRead}
              disabled={
                isMarkingAllAsRead || isClearingAll || isProcessingAction
              }
            >
              {isMarkingAllAsRead
                ? "Marking all as read..."
          ...
          {notificationsList.length > 0 && (
            <button
              className="text-[#EF4444] font-medium cursor-pointer"
              ...
              <div className="text-sm w-[80%]">
                {!notif.is_read && (
                  <button
                    ...
                    // UPDATED: Disable only if the current notification is being processed
                    disabled={
                      isMarkingAllAsRead ||
                      isClearingAll ||
                      isProcessingAction === notif._id
                    }
                  >
                    {/* UPDATED: Change text only for the notification being processed */}
                    {isProcessingAction === notif._id
                      ? "Marking as read..."
                      : "Mark as read"}
                  </button>
