...
// Ensures percent change is correct, handles numbers only
const calculateChange = (currentValue, previousValue) => {
  ...
  // If previous is zero and current is positive, show "+100%"
  if (previousValue === 0) {
      ...
      try {
        ...
        if (response.status && response.data) {
          ...
          const loansChange = calculateChange(
            ...
            dashboardData.prev_total_loans_disbursed
          );
          const usersChange = calculateChange(
            dashboardData.active_users,
            dashboardData.prev_active_users
          );
          const repaymentChange = calculateChange(
            dashboardData.repayment_rate,
            dashboardData.prev_repayment_rate
          );
          ...
          const formattedStats = [
            {
              label: "Total Loans Disbursed",
              value: `₦ ${
                Number(dashboardData.total_loans_disbursed).toLocaleString() ||
                "0"
              }`,
              ...
              period: "this month",
              chartType: "loans", // Correct chartType for loans
            },
            {
              label: "Active Users",
              value: Number(dashboardData.active_users).toLocaleString() || "0",
              ...
              period: "this month",
              chartType: "users", // Correct chartType for users
            },
            {
              label: "Repayment Rate",
              value: `${
                dashboardData.repayment_rate !== undefined &&
                dashboardData.repayment_rate !== null
                  ? Number(dashboardData.repayment_rate).toFixed(1)
                  : "0"
              }%`,
              ...
              period: "this month",
              chartType: "repayments", // Correct chartType for repayments
            },
          ];
        ...
        {stats.map((stat, index) => (
          <div key={index} className="bg-white rounded-lg shadow">
            <div className="items-center">
              ...
              <div
                className={`flex items-center text-sm px-6 py-2 ${
                  stat.trend === "up" ? "text-[#16A34A]" : "text-[#EF4444]"
                }`}
              >
              ...
              <div className="mt-3">
                <DashboardChart data={stat.chartData} trend={stat.trend} />
              </div>
            </div>
          </div>
        ))}
