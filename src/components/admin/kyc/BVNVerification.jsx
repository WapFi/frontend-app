...
function BVNVerification() {
  ...
  useEffect(() => {
    const fetchBVNs = async () => {
      ...
      setError(null);
      try {
        const params = { type: "bvn", page: currentPage, limit: itemsPerPage };
        if (debouncedSearchTerm) params.search = debouncedSearchTerm;
        if (startDate) {
          const d = startDate;
          params.start_date = `${d.getFullYear()}-${(d.getMonth() + 1)
            .toString()
  ...
  return (
    <div className="space-y-6">
      {/* Page header (with mobile search fix) */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between">
        <div className="flex justify-between items-center">
          ...
          <div className="relative md:hidden text-[rgba(34,34,34,0.60)]">
            ...
            />
            {startDate && (
              <span className="absolute left-8 top-2.5 text-gray-900 bg-white pr-2 pointer-events-none select-none">
        ...
        <div className="hidden md:flex justify-between gap-3 w-full">
          <div className="relative w-full md:w-[20%] flex items-center text-[rgba(34,34,34,0.60)]">
            <DatePicker
              ...
              maxDate={new Date()}
              placeholderText="Select Date"
              className="pl-8 pr-3 py-2 border border-[rgba(0,0,0,0.08)] rounded-2xl text-sm focus:outline-none text-[rgba(34,34,34,0.60)] focus:ring-1 focus:ring-yellow-500 focus:border-yellow-500 w-full cursor-pointer"
              calendarClassName="z-50"
            />
            <img
              src={calendarIcon}
